...
class DataLoader(object):
    ...
    def load_features_labels(self, features_file, labels_file, d):
        """
        Loads features and labels from their corresponding files. Maps tokens and tags to their indices and stores
        ...
        features = []
        labels = []
        #print("load_features_labels here!!!")
        ...
        #    assert len(labels[i]) == len(features[i])
    ...
    def load_data(self, types, data_dir):
        ...
        for split in ['train', 'val', 'test']:
            if split in types:
                features_file = os.path.join(data_dir, split, "features.txt")
                labels_file = os.path.join(data_dir, split, "labels.txt")
                data[split] = {}
                self.load_features_labels(features_file, labels_file, data[split])
    ...
    def data_iterator(self, data, params, shuffle=False):
        ...
        # one pass over data
        #print((data['size']+1)//params.batch_size)
        for i in range((data['size']+1)//params.batch_size):
            # fetch sentences and tags
            raw_batch_features = [data['data'][idx] for idx in order[i*params.batch_size:(i+1)*params.batch_size]]
            raw_batch_labels = [data['labels'][idx] for idx in order[i*params.batch_size:(i+1)*params.batch_size]]

            # compute length of longest sentence in batch
            #batch_max_len = max([len(s) for s in batch_features])
            ...
            #batch_data = self.pad_ind*np.ones((len(batch_features), batch_max_len))
            #batch_data = np.ones((len(batch_features), batch_max_len))
            #batch_labels = -1*np.ones((len(batch_features), 1))
            
            
            # copy the data to the numpy array
            '''
            for j in range(len(batch_features)):
                cur_len = len(batch_features[j])
                # list to numpy
                batch_data[j][:cur_len] = batch_features[j]
                batch_labels[j][:cur_len] = batch_tags[j]
            '''
            for i in range(len(raw_batch_features)):
                raw_batch_features[i] = raw_batch_features[i].split(',')
                raw_batch_features[i] = [float(j) for j in raw_batch_features[i]]
            raw_batch_labels = [int(i) for i in raw_batch_labels]
            
            batch_data = np.array(raw_batch_features)
            batch_labels = np.array(raw_batch_labels)
            #print(batch_data)
            #print(batch_labels)
